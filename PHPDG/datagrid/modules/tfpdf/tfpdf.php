...
// [#004] added by ApPHP 20.09.2012
if(!defined('tFPDF_VERSION')) define('tFPDF_VERSION','1.24');
...
*                                                                              *
*******************************************************************************/
// changed by ApPHP 20.04.2019 from tFPDF to __construct()
...
//Colored table 
function FancyTable($header,$data,$type,$caption='')
{
    ...
    // [#006] added by ApPHP 04.10.2012
    if(!empty($caption)){
        $this->Cell(200,10,$caption,0,0,'C',false);
        $this->Ln();
    }
    ...
    $col_width = (($type == "columnar") ? 100 : (int)(200 / $col_count));
    for($i=0;$i<$col_count;$i++){
        // [#002] added by ApPHP 05.09.2010
        $this->Cell($col_width,7,$header[$i],1,0,'L',true);
    }
    $this->Ln();
    ...
    // [#002] added by ApPHP 05.09.2010
    if($type == "tabular") $this->Cell(array_sum($w),0,'','T');
}
...
function Output($name='', $dest='', $debug='')
{
	...
	switch($dest)
	{
		...
		case 'F':
			// Save to local file
			// [#001] changed by ApPHP 05.11.2013
			$f = @fopen($name,'wb');
			if(!$f){                               
				if($debug == true){
                    ...
                    $this->Error('Unable to create output file: '.$name.'. <br />'.$error_message);
                }else{
                    $this->Error("<span style='color:#a61100'>Unable to create output file. <br />For more information turn on debug mode.</span>");
                }
			}
...
function _putfonts()
{
	...
	foreach($this->fonts as $k=>$font)
	{
		...
		// TrueType embedded SUBSETS or FULL
			...
			$subset = $font['subset'];
			unset($subset[0]);
            // [#005] - ApPHP - 30.09.2012
			$ttfontstream = $ttf->makeSubset($this->fontpath.'unifont/DejaVuSansCondensed.ttf', $subset);
